...
export const fetchPosts = (page=1):any => {
  return async (dispatch: Dispatch<PostByUserAction>, getState: ()=> RootState) => {
    try {
      ...
      if (page===1){
        dispatch({type: PostByUserActionTypes.SET_TOTAL_POSTS, payload: response.headers['x-total-count']})
        dispatch({type:PostByUserActionTypes.FETCH_POSTS_SUCCESS, payload:  response.data})
      }
      else{
        dispatch({type:PostByUserActionTypes.FETCH_POSTS_SUCCESS, payload:  [...state.postsByUser.posts,...response.data]})
      }
    } catch (error) {
      dispatch({type: PostByUserActionTypes.FETCH_POSTS_FAIL, payload: error})
    }
...
console.log(postsByUser)
    ...
    if (!newPosts.likes.includes(userId)) {
      ...
      newPosts.likes = newPosts.likes.filter(
        (item: number) => item !== userId
      );
    }
    try {
      const response = await axios.put(`http://localhost:3000/postsByUser/${idPost}`, {
        ...newPosts,
      });
      dispatch({
      ...
      dispatch({ type: PostByUserActionTypes.MUTATE_POSTS_FAIL, payload: error });
    }
  };
};
